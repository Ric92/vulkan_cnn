...
OLP::OLP(uint32_t input_size, uint32_t batch_size) {
    setup_vulkan(instance, debugMessenger, physicalDevice, queueFamilyIndex, device, queue);

    add(256, input_size, batch_size);
    
}

void OLP::add(uint32_t layer_dim, uint32_t input_size, uint32_t batch_size) {
    ...
    Convolution* d = new Convolution(device, queueFamilyIndex, physicalDevice, this->batch_size, input_dim, layer_dim, input_buffer);
...
void OLP::eval_batch(std::vector<std::vector<float>>& batch, VkCommandBuffer& evalCommandBuffer,
...
float OLP::evaluate(std::vector<std::vector<float>> &X, std::vector<std::vector<float>> &y) {
